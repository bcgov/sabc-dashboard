...
namespace App\BcscXmlDecrypt;

use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Log;

class XmlSecurityKey extends Controller
{
    ...
    private function decryptPrivate($data)
    {
        if (! openssl_private_decrypt($data, $decrypted, $this->key, $this->cryptParams['padding'])) {
            Log::error('Possible PEM file corrupt if error message is the following: error:0906D06C:PEM routines:PEM_read_bio:no start line ... Exception(code: 0): Failure decrypting Data (openssl private) - error:0906D06C:PEM routines:PEM_read_bio:no start line ...');
    ...
    {
//        Log::warning($data);
//        Log::warning($this->cryptParams['type']);
        if ($this->cryptParams['library'] === 'openssl') {
            switch ($this->cryptParams['type']) {
                case 'symmetric':
